...
<style>
  ...
  *{box-sizing:border-box}
  ...
  optgroup{color:var(--text);font-style:normal;font-weight:700}
  option{color:var(--text);background:#11161d;font-weight:400}
  ...
  <div class="card">
    ...
    <form id="f" onsubmit="return save(event)">
      ...
      <details>
        ...
        <label>POSIX time zone string (auto-filled by the dropdown above, or type your own)</label>
...
<script>
// [ianaName, posixTz] pairs for the time zone dropdown below.
...
function tzPopulate(){
  ...
  for (var i=0;i<TZ_LIST.length;i++){
    var name = TZ_LIST[i][0];
    var slash = name.indexOf('/');
    var region = slash === -1 ? name : name.substring(0, slash);
    var label = slash === -1 ? name : name.substring(slash + 1).replace(/_/g, ' ');
    if (region !== group){
...
function tzResolve(){
  var sel = document.getElementById('tzselect');
  var entry = TZ_LIST[sel.value];
  if (entry) document.getElementById('tz').value = entry[1];
}
function tzInit(){
  tzPopulate();
  ...
  sel.addEventListener('change', tzResolve);
}
function bars(rssi){
