...
// Menu contents: Settings (WiFi, Time Zone, Date/Time, About - all plain
// text lists) and Back. WiFi scans for nearby networks, lets you pick one
// and type its password on an on-screen keyboard, then connects. Time Zone
// picks a continent, then a specific zone within it - covers the same
// ~430 IANA zones as the web setup page. Date/Time sets the clock by hand,
// field by field - used for Manual (offline) mode, or just to correct the
// time. About shows the device's current IP/hostname.
namespace Menu {
